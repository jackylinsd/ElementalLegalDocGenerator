...
def claim(thisCase):
    ...
    q1_2 = st.number_input("尚欠本金金额", key="principal_amount", placeholder="请输入尚欠本金金额", min_value=0.0, step=0.01, format="%.2f")
    q1_3 = st.radio("是否为外币", ["否", "是"], key="principal_currency", horizontal=True)
    q1_4 = ''
    if q1_3 == "是":
        q1_4 = st.text_input("外币币种", key="principal_currency_type", placeholder="请输入外币币种")
    thisCase.reply_matters.append(
        {"type": "1. 透文本金", "information": f"截至{q1_1}止，尚欠本金{q1_2:.2f}元（{'人民币，下同；如为外币需特别注明' if not q1_3 else q1_4})"}
    )

    st.subheader("2. 利息、罚息、复利、滞纳金、违约金、手续费等")
    q2_1 = st_date_input("截至日期", key="interest_due_date")
    q2_2 = st.number_input("欠利息、罚息、复利、滞纳金、违约金、手续费等金额", key="interest_amount", placeholder="请输入欠利息、罚息、复利、滞纳金、违约金、手续费等金额", min_value=0.0, step=0.01, format="%.2f")
    q2_3 = st.text_area("明细", key="interest_details", placeholder="请输入明细")
    thisCase.reply_matters.append(
        {"type": "2. 利息、罚息、复利、滞纳金、违约金、手续费等", "information": f"截至{q2_1}止，欠利息、罚息、复利、滞纳金、违约金、手续费等共计{q2_2:.2f}元；\n自{q2_1}之后的利息、罚息、复利、滞纳金、违约金以及手续费等各项费用按照信用卡领用协议计算至实际清偿之日止\n明细：{q2_3}"}
    )

    st.subheader("3. 是否主张担保权利")
    q3_1 = st.radio("是否主张担保权利", ["否", "是"], key="guarantee_rights", horizontal=True)
    if q3_1 == "是":
        q3_2 = st.text_area("内容", key="guarantee_details", placeholder="请输入担保权利内容")
        thisCase.reply_matters.append(
    ...
    q4_1 = st.radio("是否主张实现债权的费用", ["否", "是"], key="legal_fees", horizontal=True)
    if q4_1 == "是":
        q4_2 = st.text_area("费用明细", key="legal_fees_details", placeholder="请输入费用明细")
        thisCase.reply_matters.append(
...
def fact(thisCase):
    ...
    if q8_0 == "模版填写":
        q8_1 = st.text_input("信用卡卡号", key="credit_card_number", placeholder="请输入信用卡卡号")
        q8_2 = st.text_input("信用卡登记权利人", key="card_holder", placeholder="请输入信用卡登记权利人")
        ...
        thisCase.reasons.append(
            {"type": "1. 信用卡办理情况", "information": f"信用卡卡号：{q8_1}\n信用卡登记权利人：{q8_2}\n办卡时间：{q8_3}\n办卡行：{q8_4}"}
        )
    else:
    ...
    q9_1 = st.number_input("透支金额", key="overdraft_amount", placeholder="请输入透支金额", min_value=0.0, step=0.01, format="%.2f")
    q9_2 = st.text_area("利息、罚息、复利、滞纳金、违约金、手续费等的计算标准", key="interest_calculation", placeholder="请输入利息、罚息、复利、滞纳金、违约金、手续费等的计算标准")
    q9_3 = st.text_area("违约责任", key="breach_terms", placeholder="请输入违约责任")
    q9_4 = st.text_area("解除条件", key="termination_conditions", placeholder="请输入解除条件")
    thisCase.reasons.append(
        {"type": "2. 信用卡合约的主要约定", "information": f"透支金额：{q9_1:.2f}元\n利息、罚息、复利、滞纳金、违约金、手续费等的计算标准：{q9_2}\n违约责任：{q9_3}\n解除条件：{q9_4}"}
    )

    st.subheader("3. 是否对被告就信用卡合约主要条款进行提示注意、说明")
    q10_1 = st.radio("是否对被告进行提示说明", ["否", "是"], key="notice_provided", horizontal=True)
    if q10_1 == "是":
        q10_2 = st.text_area("提示说明的具体方式以及时间地点", key="notice_details", placeholder="请输入提示说明的具体方式以及时间地点")
        thisCase.reasons.append(
            {"type": "3. 是否对被告就信用卡合约主要条款进行提示注意、说明", "information": f"是☑ 提示说明的具体方式以及时间地点：{q10_2}\n否☐"}
        )
    else:
        thisCase.reasons.append(
            {"type": "3. 是否对被告就信用卡合约主要条款进行提示注意、说明", "information": "是☐ 提示说明的具体方式以及时间地点：\n否☑"}
        )

    st.subheader("4. 被告已还款金额")
    q11_1 = st.number_input("已还款金额", key="repaid_amount", placeholder="请输入已还款金额", min_value=0.0, step=0.01, format="%.2f")
    thisCase.reasons.append(
    ...
    q12_2 = st.number_input("欠付信用卡本金", key="unpaid_principal", placeholder="请输入欠付信用卡本金", min_value=0.0, step=0.01, format="%.2f")
    q12_3 = st.number_input("利息", key="unpaid_interest", placeholder="请输入利息", min_value=0.0, step=0.01, format="%.2f")
    q12_4 = st.number_input("罚息", key="unpaid_penalty", placeholder="请输入罚息", min_value=0.0, step=0.01, format="%.2f")
    q12_5 = st.number_input("复利", key="unpaid_compound_interest", placeholder="请输入复利", min_value=0.0, step=0.01, format="%.2f")
    q12_6 = st.number_input("滞纳金", key="unpaid_late_fee", placeholder="请输入滞纳金", min_value=0.0, step=0.01, format="%.2f")
    q12_7 = st.number_input("违约金", key="unpaid_penalty_fee", placeholder="请输入违约金", min_value=0.0, step=0.01, format="%.2f")
    q12_8 = st.number_input("手续费", key="unpaid_service_fee", placeholder="请输入手续费", min_value=0.0, step=0.01, format="%.2f")
    thisCase.reasons.append(
        {"type": "5. 被告逾期未还款金额", "information": f"逾期时间：{q12_0}\n截至{q12_1}，被告欠付信用卡本金：{q12_2:.2f}元、利息：{q12_3:.2f}元、罚息：{q12_4:.2f}元、复利：{q12_5:.2f}元、滞纳金：{q12_6:.2f}元、违约金：{q12_7:.2f}元、手续费：{q12_8:.2f}元"}
    )
    ...
    q14_1 = st.radio("是否签订物的担保合同", ["否", "是"], key="collateral_contract", horizontal=True)
    if q14_1 == "是":
    ...
    q16_1 = st.radio("是否最高额担保", ["否", "是"], key="maximum_guarantee", horizontal=True)
    if q16_1 == "是":
        q16_2 = st_date_input("担保债权的确定时间", key="guarantee_determination_date")
        q16_3 = st.number_input("担保额度", key="guarantee_amount", placeholder="请输入担保额度", min_value=0.0, step=0.01, format="%.2f")
        thisCase.reasons.append(
    ...
    q17_1 = st.radio("是否办理抵押、质押登记", ["否", "是"], key="collateral_registration", horizontal=True)
    if q17_1 == "是":
        q17_2 = st.radio("登记类型", ["正式登记", "预告登记"], key="registration_type", horizontal=True)
        thisCase.reasons.append(
            {"type": "10. 是否办理抵押、质押登记", "information": f"是☑\n登记类型：{'正式登记☑ 预告登记☐' if q17_2 == '正式登记' else '正式登记☐ 预告登记☑'}"}
        )
    else:
        thisCase.reasons.append(
    ...
    q18_1 = st.radio("是否签订保证合同", ["否", "是"], key="guarantee_contract", horizontal=True)
    if q18_1 == "是":
        q18_2 = st_date_input("签订时间", key="guarantee_contract_date")
        q18_3 = st.text_input("保证人", key="guarantee_person", placeholder="请输入保证人")
        q18_4 = st.text_area("主要内容", key="guarantee_content", placeholder="请输入主要内容")
        thisCase.reasons.append(
    ...
    q19_1 = st.radio("保证方式", ["无","一般保证", "连带责任保证"], key="guarantee_type", horizontal=True)
    if q19_1 == "无":
        thisCase.reasons.append(
        ...
        thisCase.reasons.append(
        {"type": "12. 保证方式", "information": f"{'一般保证☑\n连带责任保证☐' if q19_1 == '一般保证' else '一般保证☐\n连带责任保证☑'}"}
    )

    st.subheader("13. 其他担保方式")
    q20_1 = st.radio("是否存在其他担保方式", ["否", "是"], key="other_guarantee", horizontal=True)
    if q20_1 == "是":
        q20_2 = st.text_input("担保形式", key="other_guarantee_form", placeholder="请输入担保形式")
