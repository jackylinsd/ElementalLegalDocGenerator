...
def claim(thisCase):
    ...
    q1_2 = st.number_input("尚欠物业费金额", key="property_fee_amount", placeholder="请输入尚欠物业费金额")
    thisCase.reply_matters.append(
        {"type": "1. 物业费", "information": f"截至{q1_1}止，尚欠物业费{q1_2}元"}
    )

    st.subheader("2. 违约金")
    q2_1 = st.radio("是否请求支付至实际清偿之日止", ["是", "否"], key="penalty_until_payment", horizontal=True)
    thisCase.reply_matters.append(
        {"type": "2. 违约金", "information": f"是否请求支付至实际清偿之日止：{'是☑ 否☐' if q2_1 == '是' else '是☐ 否☑'}"}
    )
...
def fact(thisCase):
    ...
    q6_1 = st.text_input("合同名称", key="contract_name", placeholder="请输入合同名称")
    q6_2 = st.text_input("合同编号", key="contract_number", placeholder="请输入合同编号")
    q6_3 = st_date_input("签订时间", key="contract_date")
    q6_4 = st.text_input("签订地点", key="contract_location", placeholder="请输入签订地点")
    thisCase.reasons.append(
        {"type": "1. 物业服务合同或前期物业服务合同签订情况", "information": f"合同名称：{q6_1}\n合同编号：{q6_2}\n签订时间：{q6_3}\n签订地点：{q6_4}"}
    )

    st.subheader("2. 签订主体")
    q7_1 = st.text_input("业主/建设单位", key="owner", placeholder="请输入业主/建设单位")
    q7_2 = st.text_input("物业服务人", key="property_service_provider", placeholder="请输入物业服务人")
    thisCase.reasons.append(
        {"type": "2. 签订主体", "information": f"业主/建设单位：{q7_1}\n物业服务人：{q7_2}"}
    )

    st.subheader("3. 物业项目情况")
    q8_1 = st.text_input("坐落位置", key="property_location", placeholder="请输入坐落位置")
    ...
    thisCase.reasons.append(
        {"type": "3. 物业项目情况", "information": f"坐落位置：{q8_1}\n面积：{q8_2}\n所有权人：{q8_3}"}
    )

    st.subheader("4. 约定的物业费标准")
    q9_1 = st.text_input("物业费标准", key="property_fee_standard", placeholder="请输入物业费标准")
    thisCase.reasons.append(
    ...
    q11_1 = st.text_input("物业费支付方式", key="property_fee_payment_method", placeholder="请输入物业费支付方式")
    thisCase.reasons.append(
        {"type": "6. 约定的物业费支付方式", "information": q11_1}
    )

    st.subheader("7. 约定的逾期支付物业费违约金标准")
    q12_1 = st.text_input("违约金标准", key="penalty_standard", placeholder="请输入违约金标准")
    thisCase.reasons.append(
        {"type": "7. 约定的逾期支付物业费违约金标准", "information": q12_1}
    )

    st.subheader("8. 被告欠付物业费数额及计算方式")
    q13_1 = st.number_input("欠付物业费数额", key="unpaid_property_fee", placeholder="请输入欠付物业费数额")
    q13_2 = st.text_area("具体计算方式", key="calculation_method", placeholder="请输入具体计算方式")
    thisCase.reasons.append(
        {"type": "8. 被告欠付物业费数额及计算方式", "information": f"欠付物业费数额：{q13_1}元；具体计算方式：{q13_2}"}
    )

    st.subheader("9. 被告应付违约金数额及计算方式")
    q14_1 = st.number_input("应付违约金数额", key="penalty_amount", placeholder="请输入应付违约金数额")
    q14_2 = st.text_area("具体计算方式", key="penalty_calculation", placeholder="请输入具体计算方式")
    thisCase.reasons.append(
        {"type": "9. 被告应付违约金数额及计算方式", "information": f"应付违约金数额：{q14_1}元；具体计算方式：{q14_2}"}
    )

    st.subheader("10. 催缴情况")
    q15_1 = st.text_area("催缴情况", key="collection_details", placeholder="请输入催缴情况")
    thisCase.reasons.append(
