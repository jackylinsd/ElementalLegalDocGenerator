...
def claim(thisCase):
    ...
    st.subheader("2. 利息（复利、罚息）")
    q2_1 = st_date_input("截至日期", key="interest_due_date")
    q2_2 = st.number_input(
        "欠利息金额", key="interest_amount", placeholder="请输入欠利息金额")
    q2_3 = st.number_input(
        "欠复利金额", key="compound_interest_amount", placeholder="请输入欠复利金额")
    q2_4 = st.number_input(
        "欠罚息（违约金）金额", key="penalty_amount", placeholder="请输入欠罚息（违约金）金额")
    q2_5 = st.text_area("计算方式", key="interest_calculation",
                        placeholder="请输入计算方式")
    q2_6 = st.radio("是否请求支付至实际清偿之日止", [
                    "是", "否"], key="interest_until_payment", horizontal=True)
    thisCase.reply_matters.append(
        {"type": "2. 利息（复利、罚息）", "information": f"截至{q2_1}止，欠利息{q2_2}元、复利{q2_3}元、罚息（违约金）{
            q2_4}元；计算方式：{q2_5}；是否请求支付至实际清偿之日止：{"是☑ 否☐" if q2_6 == '是' else '是☐  否☑'}"}
    )
...
def fact(thisCase):
    ...
    q9_1 = st.text_input("合同名称", key="contract_name", placeholder="请输入合同名称")
    q9_2 = st.text_input("合同编号", key="contract_number", placeholder="请输入合同编号")
    q9_3 = st_date_input("签订时间", key="contract_date")
    q9_4 = st.text_input("签订地点", key="contract_location",
                         placeholder="请输入签订地点")
    ...
    q11_2 = st.number_input(
        "实际发放金额", key="actual_amount", placeholder="请输入实际发放金额")
    thisCase.reasons.append(
        {"type": "3. 借款金额", "information": f"约定借款金额：{q11_1}元；实际发放金额：{q11_2}元"}
    )
    ...
    thisCase.reasons.append(
        {"type": "4. 借款期限", "information": f"是否到期：{
            '是☑ 否☐' if q12_1 == '是' else '是☐ 否☑'}；约定期限：{q12_2}至{q12_3}"}
    )

    st.subheader("5. 借款利率")
    q13_1 = st.number_input(
        "利率（%/年）", key="interest_rate", placeholder="请输入利率")
    q13_1_2 = st.text_input(
        "合同依据", key="interest_rate_description", placeholder="请输入合同中的第几条")
    q13_2 = st.number_input(
        "逾期上浮利率（%/年）", key="overdue_rate", placeholder="请输入逾期上浮利率")
    q13_2_2 = st.text_input(
        "合同依据", key="overdue_rate_description", placeholder="请输入合同中的第几条")
    q13_3 = st.number_input(
        "复利（%/年）", key="compound_interest_rate", placeholder="请输入复利")
    q13_3_2 = st.text_input(
        "合同依据", key="compound_interest_rate_description", placeholder="请输入合同中的第几条")
    q13_4 = st.number_input(
        "罚息（违约金）（%/年）", key="penalty_rate", placeholder="请输入罚息（违约金）")
    q13_4_2 = st.text_input(
        "合同依据", key="penalty_rate_description", placeholder="请输入合同中的第几条")
    thisCase.reasons.append(
        {
            "type": "5. 借款利率",
            "information": (
                f"利率{'☑：' + str(q13_1) if q13_1 !=
                     0 else '☐：'}%/年（合同条款：{q13_1_2}）；\n"
                f"逾期上浮利率{'☑：' + str(q13_2) if q13_2 !=
                         0 else '☐：'}%/年（合同条款：{q13_2_2}）；\n"
                f"复利{'☑：' + str(q13_3) if q13_3 !=
                     0 else '☐：'}%/年（合同条款：{q13_3_2}）；\n"
                f"罚息（违约金）{'☑：' + str(q13_4) if q13_4 !=
                          0 else '☐：'}%/年（合同条款：{q13_4_2}）"
            )
        }
    )

    st.subheader("6. 借款发放时间")
    q14_1 = st_date_input("发放时间", key="loan_disbursement_date")
    q14_2 = st.number_input(
        "发放金额", key="disbursement_amount", placeholder="请输入发放金额")
    thisCase.reasons.append(
        {"type": "6. 借款发放时间", "information": f"{q14_1}，发放{q14_2}元"}
    )
    ...
    thisCase.reasons.append(
        {"type": "7. 还款方式", "information": f"还款方式：{q15_r}"}
    )
    ...
    thisCase.reasons.append(
        {"type": "8. 还款情况", "information": f"已还本金：{
            q16_1}元；已还利息：{q16_2}元，还息至{q16_3}"}
    )
    ...
    if q22_1 == "是":
        ...
        thisCase.reasons.append(
            {"type": "14. 是否签订保证合同", "information": f"是☐ \n签订时间：\n保证人：\n主要内容：\n否☑"}
        )
    ...
    if q23_1 == "无":
        ...
        thisCase.reasons.append(
            {"type": "15. 保证方式", "information": f"保证方式：{
                '一般保证☑\n连带责任保证☐'if q23_1 == '一般保证' else '一般保证☐\n连带责任保证☑'}"}
        )

    st.subheader("16. 其他担保方式")
    q24_1 = st.radio("是否存在其他担保方式", ["否", "是"],
    ...
    q26_1 = st.text_area(
        "证据清单（可另附页）", key="evidence_list", placeholder="请输入证据清单（可另附页）"
    )
    thisCase.reasons.append(
...
# 定义数据格式化器
class FinancialLoanCaseFormatter(BaseCaseFormatter):
    """数据格式化器"""
    ...
    def format_case(thisCase):
        ...
        template_data = super(FinancialLoanCaseFormatter,
                              FinancialLoanCaseFormatter).format_case(thisCase)
